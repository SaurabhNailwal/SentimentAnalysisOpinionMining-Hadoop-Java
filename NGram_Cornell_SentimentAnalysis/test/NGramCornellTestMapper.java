...
public class NGramCornellTestMapper extends MapReduceBase implements
	...
	public void map(LongWritable key, Text value,
		...
		String line = removeStopWords(value.toString());
		String[] items = line.toLowerCase().split("\\s+");
		...
		for (int i = 0; i < length - 2; i++) {
			for (int j = i + 1; j < length - 1; j++) {
				for (int k = j + 1; k < length; k++) {
					outValue.set(items[i] + "," + items[j] + "," + items[k]);
					System.out.println("triplet:" + outValue.toString());
					count++;
					output.collect(new IntWritable(count), outValue);
		...
		for (int i = 0; i < length - 1; i++) {
			for (int j = i + 1; j < length; j++) {
				outValue.set(items[i] + "," + items[j]);
				System.out.println("doublet:" + outValue.toString());
				count++;
				output.collect(new IntWritable(count), outValue);
			}
		}
			...
			remStopWords += (" " + items[i].toLowerCase().trim() + " ")
					.replaceAll(
							" a | a's | able | about | above | according |"
									...
									+ " by | c | c'mon | c's | came | can | cause | causes | certain | certainly |"
									...
									+ " v | value | various | via | viz | vs | w | want | wants | was | wasn't | way | we | we'd | we'll |"
